...
- [各画面・機能説明](#各画面機能説明)
  ...
  - [ユーザー登録画面(/register)](#ユーザー登録画面register)
    ...
    - [画面遷移](#画面遷移-1)
  - [Home画面(/home)](#home画面home)
    - [ユーザー情報カード](#ユーザー情報カード)
    - [お知らせ機能](#お知らせ機能)
    - [最近の日記リスト](#最近の日記リスト)
    - [画面遷移](#画面遷移-2)
  - [日記一覧画面(/diaries)](#日記一覧画面diaries)
    - [検索設定](#検索設定)
    - [日記一覧とページネーション](#日記一覧とページネーション)
    - [日記の操作（編集・削除）](#日記の操作編集削除)
  - [日記作成画面](#日記作成画面)
    - [新規作成・更新モード共通の機能](#新規作成更新モード共通の機能)
    - [新規作成モード(/diary/new)](#新規作成モードdiarynew)
    - [更新モード(/diary/:id)](#更新モードdiaryid)
...
* もっとしっかりとしたユーザー登録機能の実装。（パスワード確認用フォームの実装等）
* 管理者アカウントを含めたユーザーアカウント管理。
...
* Gemini API 連携（コメント自動生成）
  ...
  このアプリの最大の特徴です。日記の内容を入力した後、「Geminiにコメントをもらう」ボタン をクリックすると、GoogleのGemini API(gemini-2.5-flash)にリクエストが送信されます。
